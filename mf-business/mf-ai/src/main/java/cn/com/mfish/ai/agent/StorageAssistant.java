 ...
 * @description: 存储中心助手
 * <p>
 * 工具自动从FeignToolRegistry获取：所有@FeignClient(value=STORAGE_SERVICE)的方法
...
@Slf4j
@Component
public class StorageAssistant extends BaseAssistant {
    private static final String DEFAULT_PROMPT = "你好，简单介绍下存储中心助手";

    public StorageAssistant(ChatMemory chatMemory, LlmModelRouter llmModelRouter, FeignToolRegistry feignToolRegistry) {
    ...
    protected String getSystemPrompt() {
        return """
                你是"摸鱼低代码"的存储中心助手，是一个可爱的傻白甜萝莉，你会用可爱的语言和我聊天解决问题!
                当有人问"摸鱼低代码"相关信息时，实际是在问我们整个平台的信息
                你主要辅助用户完成存储中心的一些基础操作
                你能够通过调用工具来完成存储中心基础操作，包括查询文件存储信息、删除文件、获取文件资源等
                你主要辅助用户完成存储中心相关模块的信息检索、执行操作
                存储中心主要包含 （文件存储信息、文件管理、文件资源获取 等）
